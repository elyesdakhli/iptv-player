...
export const getStreams = async (
  ...
  try {
    const action =
      mode === "TV"
        ? TV_CATEGORY_STREAMS_API_ACTION
        : VOD_CATEGORY_STREAMS_API_ACTION;
    let url =
      source.url +
      "/" +
      PLAYER_API_PATH +
...
export const getVodStreamInfo = async (
  source: Source,
  streamId: string
  ...
  try {
    ...
    return Promise.resolve({
      ...
      country: vodResponseInfo.country,
...
const mapAllVodStreamResponseToVodStream = (
  streams: VodStreamResponse[]
): VodStream[] => {
  return streams.map((stream: VodStreamResponse) =>
    mapStreamResponseToVodStream(stream)
  );
};
