        ...
        mode === 'TV' ?
            <>
                {streams?.map((stream, index) => (
                    <TvStreamCard stream={stream} index={index} key={index} onSelect={onSelect} />
                ))}
            </> :
            <>
                {streams?.map((stream, index) => (
...
const TvStreamCard = ({stream, index, onSelect}: StreamCardProps) => {
    ...
    return (
        <Col xs={12} sm={6} md={4} lg={3}>
            <Card className={`${( hoveredStreamInd == index)? "border-primary shadow-lg" : "border-light"}`}
...
const FilmStreamCard = ({stream, onSelect}: { stream: Stream, onSelect: (stream: Stream) => void }) => {
